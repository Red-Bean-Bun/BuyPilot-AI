...
> **来源**：`doc/prd/03-商品详情接口PRD.md`
> **状态**：P1 待实现
> **日期**：2026-06-03

### A.1 背景

当前 `ProductHeroDetailScreen` 只展示 SSE `product_card` 中的基础信息（图片、名称、价格、品牌、标签、AI 推荐理由、风险提示）。原始数据中丰富的 `marketing_description`、`official_faq`、`user_reviews`、`sku_options` 完全未送到前端。新增 `GET /products/{product_id}` REST 接口，Android 端投机预取 + 缓存，详情页秒开。

### A.2 数据获取与预取策略

**接口**：`GET /products/{product_id}` → `ProductDetailResponse`

**预取时机**：收到 SSE `product_card` 事件时立即触发预取（`ChatRepository.prefetchProductDetail`）。

**缓存**：内存 `LruCache<String, ProductDetailResponse>(20)`，key = `product_id`。

**降级**：REST 失败时详情页回退到 SSE `ProductPayload` 数据，不显示 loading，不崩溃。
...
在 `CinematicProductDetailPanel` 中，**已有模块**与**新增模块**交错排列：

| # | 模块 | 状态 | 数据源 |
|---|------|------|--------|
| 1 | 商品大图 + 渐变遮罩 | ✅ 已有 | `product.image_url` |
| 2 | 价格 + 名称 + 品牌 | ✅ 已有 | `product.name/price/brand` |
| 3 | 标签行（肤质/成分/场景） | ✅ 已有 | `product.skin_type_match` 等 |
| 4 | **SKU 规格选择器** | 🆕 新增 | `product.sku_options` |
| 5 | AI 推荐理由 | ✅ 已有 | `reason` |
| 6 | **商品亮点** | 🆕 新增 | `highlights` |
| 7 | 风险提示 | ✅ 已有 | `risk_notes` |
| 8 | 属性行（适用对象/成分/场景） | ✅ 已有 | `product.*` |
| 9 | **商品描述（可折叠）** | 🆕 新增 | `marketing_description` |
| 10 | **官方 FAQ** | 🆕 新增 | `faqs` |
| 11 | **用户评价** | 🆕 新增 | `reviews` |
...
- **布局**：`LazyRow`，横向滚动，`spacedBy(10.dp)`
- **每个 SKU 标签**：`Surface` 圆角卡片，显示属性 + 价格（如 "30ml 经典装 ¥720"）
- **选中态**：`BuyPilotColors.Primary` 边框 1.5dp + `Primary.copy(alpha = 0.08f)` 背景
- **未选中态**：`Border` 边框 1dp + `SurfaceCard` 背景
- **默认选中**：第一个 SKU
- **交互**：切换 SKU 时更新顶部价格显示（动画 `tween(200ms)`）

#### A.4.2 商品亮点

- **标题**："商品亮点"，`ImmersiveSectionTitle` 样式
- **列表**：`Column`，`spacedBy(10.dp)`
- **每条**：`Row` = ✅ emoji + `Text(highlight, TextSecondary, Body)`
- **无数据时**：整块隐藏

#### A.4.3 商品描述（可折叠）

- **标题**："商品描述"，`ImmersiveSectionTitle` 样式
- **折叠态**：`maxLines = 3`，`TextOverflow.Ellipsis`，底部显示 "展开全文" 按钮
- **展开态**：显示全文 + "收起" 按钮
- **动画**：`animateContentSize(tween(260ms))`
- **无数据时**：整块隐藏

#### A.4.4 官方 FAQ

- **标题**："官方问答"，`ImmersiveSectionTitle` 样式
- **列表**：`Column`，每条 `Surface` 卡片
- **折叠态**：只显示 question + 右侧 ▶ 箭头图标
- **展开态**：question + answer + ▼ 箭头
- **互斥模式**：同时只展开一条（`remember { mutableStateOf(-1) }` 追踪展开索引）
- **动画**：`animateContentSize(tween(200ms))`
- **无数据时**：整块隐藏

#### A.4.5 用户评价

- **标题**："用户评价 (N)"，`ImmersiveSectionTitle` 样式，N = 评价数量
- **评分分布条**：5 行水平条形图（5星→1星），每行 = 星级标签 + 比例条 + 数量
  - 比例条用 `Box` + `fraction = count / total`，颜色 `Primary`
- **评价列表**：`Column`，`spacedBy(12.dp)`
- **每条评价**：
  - 第一行：昵称（`TextMuted, Label`）+ 评分星星（`star` / `star_border` Material Icons）
  - 第二行：评价内容（`TextSecondary, Body`），`maxLines = 3`，`TextOverflow.Ellipsis`
- **无数据时**：整块隐藏
