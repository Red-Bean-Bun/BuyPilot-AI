...
| 6 | LLM 接入方式 | **task-oriented interface（analyze_intent / generate_criteria / generate_recommendation / analyze_image），内部按 TASK_MODEL_MAP 选 primary/fallback** | 每个接口返回结构化 Pydantic model，禁止返回 raw str |
...
| 16 | 模型切换机制 | **task-oriented interface，settings.py 定义 TASK_MODEL_MAP，每个接口返回结构化 Pydantic model，禁止返回 raw str** | analyze_intent / generate_criteria / generate_recommendation / analyze_image 各接口按 TASK_MODEL_MAP 选 primary，primary 失败自动 retry fallback |
...
```Python
async def chat_stream(session_id, user_input, image_url=None, history=None):
    turn_id = new_turn_id()
    deck_id = f"deck_{turn_id}"
    seq = EventSeq(turn_id)  # seq 递增 + event_id 生成器

    # ====== T0: 立即响应 ======
    yield ThinkingEvent(
        session_id=session_id, turn_id=turn_id,
        seq=seq.next(), event_id=seq.event_id(),
        node_id=f"thinking_{turn_id}",
        created_at_ms=now_ms(),
        stage="understanding", message="正在理解您的需求...",
    )
    ...
    if slot_result.needs_clarification:
        yield ClarificationEvent(
            ...
            suggested_options=slot_result.suggested_options,
        )
        yield DoneEvent(
            session_id=session_id, turn_id=turn_id,
            seq=seq.next(), event_id=seq.event_id(),
    ...
    yield ThinkingEvent(
        ...
        node_id=f"thinking_{turn_id}",
        created_at_ms=now_ms(),
        stage="criteria", message="正在生成购买标准...",
    )
    ...
    yield CriteriaCardEvent(
        ...
        created_at_ms=now_ms(),
        editable=True,
        criteria=criteria,  # CriteriaPayload Pydantic model
    ...
    yield ThinkingEvent(
        ...
        node_id=f"thinking_{turn_id}",
        created_at_ms=now_ms(),
        stage="searching", message="找到5个匹配商品...",
    )
    ...
    for chunk in generate_recommendation_stream(criteria, ranked, evidences):
        if chunk.type == "text":
            yield TextDeltaEvent(
                ...
                created_at_ms=now_ms(),
                message_id=f"msg_{turn_id}", delta=chunk.content,
                done=False,
            )
            ...
            yield ProductCardEvent(
                ...
                created_at_ms=now_ms(),
                rank=chunk.rank,
                product=chunk.product,  # ProductPayload
                reason=chunk.reason,
...
class PipelineState(TypedDict):
    ...
    history: List[Dict]
    intent: Optional[IntentResult]        # 结构化意图（Pydantic model，待定义）
    criteria: Optional[CriteriaPayload]   # 购买标准（src/types/sse_events.py 已定义）
    needs_clarification: bool
    clarification_questions: Optional[List[str]]
    filters: Optional[Constraints]        # 硬过滤条件（Constraints DSL，src/types/sse_events.py 已定义）
    soft_preferences: Optional[Dict]      # 软偏好（向量语义处理，不进 SQL）
...
> 注意：envelope 字段（schema_version / session_id / turn_id / seq / event_id / node_id / deck_id / display_mode / created_at_ms）定义在 `SSEEventBase` 基类上；业务字段（如 ThinkingEvent 的 stage + message）直接作为各子类的属性，不包裹在 payload 子对象中。这是 Pydantic discriminated union 的标准做法。
...
- `seq`：同一 `turn_id` 内单调递增，由 `EventSeq(turn_id).next()` 生成，客户端按它排序。
- `event_id`：幂等去重 key，由 `EventSeq(turn_id).event_id()` 生成，格式 `${turn_id}:${seq}`。
- `node_id`：聊天流节点稳定 key，用于 upsert，而不是 append-only。
- `deck_id`：商品卡堆稳定 key；同一轮多个 `product_card` 必须共用同一 `deck_id`。
- `display_mode`：后端声明展示语义，每个 SSEEvent 子类有固定默认值（ThinkingEvent → inline_thinking，CriteriaCardEvent → summary_card 等），允许值包括 `inline_thinking`、`inline_card`、`inline_text`、`summary_card`、`swipe_deck_item`、`none`。
- 业务字段：各事件子类的独有属性直接定义在 Pydantic model 上（如 ThinkingEvent.stage + ThinkingEvent.message），不包裹在 payload 子对象中。

**事件业务字段规范（各 SSEEvent 子类的独有属性）：**

```Plain Text
ThinkingEvent:
  display_mode: inline_thinking  (固定默认值)
  node_id: thinking_{turn_id}
  stage: "understanding|clarifying|criteria|searching|reranking|generating|deciding"
  message: "..."

ClarificationEvent:
  display_mode: inline_card  (固定默认值)
...
CriteriaCardEvent:
  display_mode: summary_card  (固定默认值)
  node_id: criteria_{criteria_id}
  editable: bool = True
  criteria: CriteriaPayload {criteria_id, category, summary, chips, constraints(Constraints DSL)}
  quick_actions: list[QuickActionPayload]

TextDeltaEvent:
  display_mode: inline_text  (固定默认值)
...
ProductCardEvent:
  display_mode: swipe_deck_item  (固定默认值)
  node_id: product_{product_id}
  deck_id: deck_{turn_id}  (必填，非可选)
  rank: int
...
# 反馈注入：更新 Constraints 封闭 DSL，禁止 dict.append
criteria.constraints = criteria.constraints.model_copy(update={
    "ingredient_avoid": criteria.constraints.ingredient_avoid + ["含酒精"],
})
```
...
│   │   ├── sse_events.py        # SSE 事件类型 + SSEEventBase envelope + Constraints DSL + CriteriaPayload + EventSeq + now_ms
