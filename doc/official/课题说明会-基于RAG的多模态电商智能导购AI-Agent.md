...
> 飞书原文：https://bytedance.larkoffice.com/docx/PupqdQ6kmoJAmaxW6U0c22RKnNb
> 抓取时间：2026-05-20
> revision_id: 457
> 本地同步说明：2026-05-28 根据队友转发的最新版课题说明补充“对比决策”场景；模型 API 明文密钥不入库，仍以 `.env.example` / 本地环境变量为准。

---

## 课题概要

利用 AI Agent + RAG 技术，构建一个多模态电商智能导购助手，将传统"展示型广告"升级为"交互型导购"，实现从内容浏览到购买决策的深度连接。
...
| **进阶** | 主动反问 | "推荐一款手机" → Agent：请问您更看重拍照、续航还是性价比？→ "拍照优先，预算 4000" | 识别信息不足时主动澄清，引导用户细化需求 |
| **高级** | 反选/排除约束 | "推荐防晒霜，但我不要含酒精的，也不要日系品牌" | 否定条件解析，从候选集中排除特定属性 |
...
> BaseAPI / Model / APIKey / Limit：见项目 `.env.example`，不外泄
>
> Trae 模型配置指南：见飞书Wiki文档 BgEHdvJTioS2UGxU2c0cCqg9n5b
...
本课题重点考察**工程能力**而非数据量。50-100 条足以验证 RAG 链路的正确性，建议精心设计数据覆盖不同类目和场景。如果场景依赖更多信息输入可以进行适当扩充相关 Case。
