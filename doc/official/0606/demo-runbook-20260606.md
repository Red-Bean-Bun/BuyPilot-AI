...
| 预期 SSE 事件 | `thinking` → `text_delta` → `product_card`×1+ → `criteria_card`(chips: 洁面类/油性肌肤/200元以内) → `done(awaiting_product_feedback)`；若只返回单个强候选，则同轮追加 `final_decision` → `done(completed)` |
...
| 前置 | 完成 Demo 1 |
| 输入话术 | `有没有更温和一点的` |
| 预期 SSE 事件 | `thinking` → `text_delta` → `product_card`×新候选 → `criteria_card`(patch: 温和/氨基酸优先) → `done(awaiting_product_feedback)`；可继续输入 `继续` 收敛到 `final_decision` |
...
| 输入话术 | `推荐一款手机` |
| 预期 SSE 事件 | `thinking` → `clarification`(required_slots: budget，suggested_options: 预算区间) → `done` |
| 预期商品类别 | 数码电子；但此轮不检索，等用户选择 |
| Android 展示点 | Clarification 卡片展示预算选项按钮；用户点击后注入下一轮 criteria |
...
| 预期 SSE 事件 | `thinking(stage=analyzing_image)` → `text_delta` → `product_card`×1+ → `criteria_card` → `done(awaiting_product_feedback)`；若只返回单个强候选，则同轮追加 `final_decision` → `done(completed)` |
...
| 预期 SSE 事件 | `thinking` → `text_delta`(场景化策略开场) → `criteria_card`(shopping_strategy.scene_type=travel) → `product_card`×多品类 → `done(awaiting_product_feedback)`；可继续输入 `继续` 收敛到 `final_decision` |
...
| 项目 | 内容 |
|------|------|
| 目标 | 验证购买意向确认闭环⭐⭐⭐：checkout_preview → checkout_confirm → audit |
| 前置 | 购物车非空（Demo 6 已加购） |
| 输入话术 | 1. `就买这个` → 2. 点击预览卡上的"确认购买意向"或输入 `确认` |
| 预期 SSE 事件 | 1. `cart_action(checkout_preview, status=success)` → 2. `cart_action(checkout_confirm, status=success)` |
| 预期展示 | 1. 确认购买意向卡（商品列表 + 总价 + "不涉及真实交易"提示）→ 2. "购买意向已确认"回执 |
