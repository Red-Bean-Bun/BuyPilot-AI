...
- `ground_truth` 用 JSON 保存不同场景的期望意图、约束、相关商品和期望 criteria chips，适配多品类、多模态、多轮场景。
...
| `ground_truth` | `JSON`, nullable | 期望答案结构，包括 `intent_type`、`constraints`、`relevant_product_ids`、`expected_criteria_chips` 等。 | 一张表支持多种评测指标，避免 schema 高频变更。 |
...
- 当前 dev.db 行数为 0，需要调用 `/admin/eval/samples/seed` 或相关脚本后才会有样本。
