...
| `filters_applied` | `JSON`, nullable | 应用的约束和诊断信息。通常包含 criteria constraints，并可能包含 `_stage_timings_ms`、`_fallbacks`。 | 记录“为什么这些商品能进候选”，同时承载运行时观测信息。 |
...
- 当前只有主键索引；查询最近 trace 按 `created_at` 排序。
