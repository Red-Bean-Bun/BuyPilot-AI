...
def initialize_database(auto_seed: bool = False, strict_embeddings: bool = False) -> dict[str, int | bool] | None:
    create_db_and_tables()
    if not auto_seed:
        return None
    expected_dimensions = EXPECTED_EMBEDDING_DIMENSIONS if strict_embeddings else None
    return seed_products_if_needed(expected_embedding_dimensions=expected_dimensions)
