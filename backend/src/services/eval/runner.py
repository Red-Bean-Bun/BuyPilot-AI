...
async def run_eval(
    ...
    for sample in samples:
        ...
        if capture.text_full and contexts:
            faith = await evaluate_faithfulness(capture.text_full, contexts)
            llm_metrics["faithfulness"] = faith["score"]

            cp = await evaluate_context_precision(sample.question, contexts)
            llm_metrics["context_precision"] = cp["score"]
            ...
            llm_metrics["hallucination_rate"] = hallu["hallucination_rate"]
        else:
            llm_metrics.update(
                {
                    "faithfulness": 1.0,
        ...
        if sample.context and capture.text_full:
            mc = await evaluate_multi_turn_consistency(
                sample.context.get("conversation_history", []),
                capture.text_full,
                contexts,
            )
            llm_metrics["multi_turn_consistency"] = mc["score"]
        else:
        ...
        all_metrics = {**det_metrics, **llm_metrics}
        all_metrics["overall_score"] = _compute_overall(all_metrics)
