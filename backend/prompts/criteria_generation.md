...
所有品类共享: budget_max, budget_min, brand_avoid, brand_prefer, origin_avoid, product_type, use_scenario。

字段含义：
- brand_avoid: 用户明确拒绝的品牌名称列表（如 ["SK-II", "资生堂"]），只在用户明确提及时填写，不推断
- brand_prefer: 用户明确偏好的品牌名称列表（如 ["华为", "Apple"]），只在用户明确提及时填写，不推断
...
7. 有 existing（历史标准）时，在它的基础上修改，不要从零开始。但以下情况必须用新值完全替换旧值，不得保留：
   - 用户明确提到新的 product_type（如从"手机"改为"电脑"），constraints.product_type 必须更新
   - 用户明确提到新的 brand_prefer（如从"华为"改为"苹果"），constraints.brand_prefer 必须更新
   - 用户消息包含可识别的产品类型关键词，即使 existing 中有不同的 product_type 也要更新
...
```json
{
  "criteria_id": "c_003",
  "category": "数码电子",
  "summary": "256G存储游戏手机，3000元以内，品牌偏好华为",
  "chips": ["256G", "游戏", "3000元内", "华为"],
  "constraints": {
    "storage": "256G",
    "screen_size": null,
    "budget_max": 3000,
